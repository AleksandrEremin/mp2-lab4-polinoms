...
Lexema Polynom::lexem(const std::string& polynomStr)
{
	...
	char* LEX_DELIMS[] = { "+","-","^","=" };//0 -для определения конца списка;
	pos;
	Lexema lexem;
	char currentChar;
	...
	while (true)
	{
		...
		case START:
			...
			else//знак ;
			{
				...
				int index = -1;
				//Lexema lexem3(LEX_NULL, LEX_NULL);
				for (int i = 0; i < 4; i++)//ищем в таблице;
				{
					//std::cout << LEX_DELIMS[i]<<"D"<<tmp;
					if (strcmp(tmp, LEX_DELIMS[i]) == 0)
					{

						index = i;
						i = 5;
						//std::cout<< "Dвыаывп1216512316516ывп";
					}
				}
				if (index != -1)
				{
	...
	for (int i = 0; i < polynomStr.length(); i++)
	{
		...
		while (currentLex.type != LEX_DELIM)
		{
			if (currentLex.type == LEX_VAR)
			{
				if (currentLex.buf == "x")
				{
					if (degX >= 1)//для записи вида 2zzz;
						degX++;
					else degX = 1;
					X = 1;
				}
				if (currentLex.buf == "y")
				{
					if (degY >= 1)//для записи вида 2zzz;
						degY++;
					else degY = 1;
					Y = 1;
				}
...
Polynom Polynom::operator+(const Polynom& _polynom) const
{
	...
	//			pCopy.monoms.Delete(tmppoltwo->data);
	//			index = 1;
	//		}
	//		tmppoltwo = tmppoltwo->next;
	//	}
	...
	//	tmpPol = tmpPol->next;
	//}
	//if (pCopy.monoms.GetSize() != 0)
	//{
	//	Node<Monom>* tmppCopy = pCopy.monoms.GetHead();;
	//	while (tmppCopy)
	//	{
	//		Res.monoms.InsertToTail(tmppCopy->data);
	//		tmppCopy = tmppCopy->next;
	//	}
	//}
}
...
Polynom Polynom::operator-(const Polynom& _polynom) const
{
	...
	while (tmppoltwo)
	{
		tmppoltwo->data = tmppoltwo->data * (-1);
		Res.monoms.InsertToTail(tmppoltwo->data);
		tmppoltwo = tmppoltwo->next;
	}
