...
Monom Monom::operator+(const Monom& _monom) const
{
	if (this->degreeX == _monom.degreeX && this->degreeY == _monom.degreeY && this->degreeZ == _monom.degreeZ)
	{
		...
		std::cout << "Разные степени моновов--------->Добавить ошибку"<<std::endl;
	}
}
...
Monom Monom::operator+(const double& _Num) const
{
	...
	{
		std::cout << "Эту ошибку вызвало сложение мономов. Файл monom.срр, строчка 83." << std::endl;
		throw "error";
	}
}
...
Monom Monom::operator-(const double& _Num) const
{
	...
	{
		std::cout << "Эту ошибку вызвало вычетание мономов. Файл monom.срр, строчка 83." << std::endl;
		throw "error";
	}
}
